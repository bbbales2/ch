...
import mahotas
import skimage.io, skimage.util, skimage.filter
...
saveStack(rescale(numpy.real(numpy.fft.ifftn(c)), 0.0, 1.0), 'gpu3/in')
...
for t in range(0, steps):
    ...
    if t % 100 == 0:
        ...
        saveStack(c, 'gpu2/out{0}/out{0}'.format(t))
...
threshold = skimage.filter.threshold_otsu(c)
out = c >= threshold
saveStack(out * 1.0, 'gpu2/seg{0}/seg{0}'.format(t))
